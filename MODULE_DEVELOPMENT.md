...
本文描述平台 `1.0.9` 的外置框架模块契约。框架模块是放在 `/modules` 中、随应用启动加载的可信 JAR；机器人插件是由 `plugin-support` 从 `/plugins` 加载并绑定到机器人的业务实现。二者不是同一个扩展层。
...
```kotlin
dependencies {
    compileOnly("com.mieai.qqbot:qqbot-module-api:1.0.9")
    compileOnly("com.mieai.qqbot:qqbot-module-spi:1.0.9")
...
- `build/distributions/qqbot-default-modules-1.0.9.zip`
- `build/distributions/qqbot-module-sdk-1.0.9.zip`
