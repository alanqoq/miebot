...
外部插件项目应依赖与宿主完全相同的 Maven 制品版本。当前平台制品版本为 `0.4.0`，Manifest 的插件 API 兼容级别为 `2.0.0`。根项目的 `pluginSdkRepository` 任务会生成可复制的本地 Maven SDK 仓库，`pluginSdkDistribution` 会把仓库、模板和本指南打成 ZIP；不需要把宿主模块或 PF4J 放进插件项目。
