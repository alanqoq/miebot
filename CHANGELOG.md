...
- 插件安装或重载后会初始化缺失的绑定配置；配置文件修复后会自动解除由配置错误引起的隔离状态。
- 缓存运行时镜像在复制新应用、模块和插件前清理旧制品，避免复用构建缓存时混入过期文件。

### Distribution

- Compose 镜像、模块制品、插件模板和 SDK 分发版本统一为 `1.0.9`；仓库继续包含下载后可直接部署所需制品与当前 Graphify 输出。
