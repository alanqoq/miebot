...
- 最新版本：[v1.0.7 Release](https://github.com/alanqoq/miebot/releases/tag/v1.0.7)
...
- Angular 22 管理后台工程
- 插件 SDK API 级别 `3.2.0`（Maven 制品 `1.0.7`，API、SPI、testkit）、可复制项目模板和本地分发任务
...
- 每个机器人/插件独立的 `/data/plugin-data/<botId>/<pluginId>/` 数据目录，由插件选择 JSON/YAML 配置文件，以及 SQLite、图片、音视频等插件自有文件
...
“机器人绑定”先按机器人显示信息、Gateway 状态、插件数和更新时间，进入编辑页后每个插件使用独立栏目，只显示插件名、右侧删除操作和文件管理器。一个插件可绑定多个机器人，但每个绑定都使用 `/data/plugin-data/<botId>/<pluginId>/` 独立目录；插件选择的 `config.json`、`config.yml` 或 `config.yaml` 只保存在该目录，不写入主数据库。新建绑定时 Web 表单加载制品的默认配置供管理员确认，保存后才创建目录和对应配置文件。文件管理器可浏览、新建、上传、下载和递归删除目录内文件，点击 `.json`、`.yml` 或 `.yaml` 文件会打开带 SHA-256 冲突检测的编辑器；同名上传默认返回冲突，Web 只有在管理员明确确认后才请求覆盖。删除插件绑定会永久删除整个绑定目录及其中的 SQLite、媒体和其他文件。多实例部署进行文件写入、解绑或删除机器人前，必须先把目标机器人和管理请求收敛到单个实例，或停掉其他副本并确认远端插件已释放文件句柄；默认单实例部署不需要额外操作。
