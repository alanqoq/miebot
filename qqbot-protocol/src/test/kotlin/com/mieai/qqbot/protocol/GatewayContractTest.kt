...
class GatewayContractTest {
    ...
    @Test
    fun decodesTypedHelloEnvelopeAndIgnoresExtensions() {
        val envelope = codec.decodeGatewayEnvelope(
            FixtureLoader.load("/fixtures/gateway/hello.json"),
    ...
    @Test
    fun retainsUnknownOpcodeForForwardCompatibility() {
        val envelope = codec.decodeGatewayEnvelope(
            FixtureLoader.load("/fixtures/gateway/unknown-opcode.json"),
            FutureGatewayData::class.java,
        )
    ...
    @Test
    fun serializesHeartbeatWithoutNullDispatchFields() {
        val heartbeat = GatewayEnvelope(GatewayOpcode.HEARTBEAT.code(), 42L, null, null, null)

        assertThat(codec.encode(heartbeat)).isEqualTo("{\"op\":1,\"d\":42}")
    }

    @Test
    fun mapsAllCurrentlyDefinedGatewayOpcodes() {
    ...
    private data class FutureGatewayData(val future: Boolean)
}
