...
class InboxControllerMockMvcTest {
    ...
    @MockitoBean
    lateinit var service: InboxAdministrationService

    @Test
    fun `rejects unauthenticated inbox read`() {
    ...
    fun `lists inbox with filters and opaque cursor without payload`() {
        `when`(
            service.list(
                "25",
                "cursor-1",
    ...
    @Test
    @WithMockUser(username = "alanqaq", roles = ["ADMIN"])
    fun `returns detail payload as text without lease internals`() {
        val rawPayload = "{\"d\":{\"content\":\"hello\"}}"
        `when`(service.get(EVENT_ID.toString())).thenReturn(
    ...
    fun `maps invalid query to the standard bad request shape`() {
        ...
        ).thenThrow(IllegalArgumentException("environment is not supported: NOPE"))
